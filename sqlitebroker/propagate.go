...
// 决策全部走 deps.go 的纯函数,与 memorybroker 语义完全一致。
func (b *Broker) propagateTx(ctx context.Context, tx *sql.Tx, id string, status taskgate.Status, now time.Time, notifs *[]taskgate.Task) error {
	...
	for len(work) > 0 {
		...
		if _, err := tx.ExecContext(ctx, `UPDATE task_deps SET done = 1 WHERE parent_id = ?`, p.id); err != nil {
			return err
		}
		// 先把直接子任务 ID 全部读出来再逐个处理:单连接上不能边扫结果集边发新语句。
		childIDs, err := childrenOf(ctx, tx, p.id)
		if err != nil {
			return err
		}
		for _, cid := range childIDs {
			c, err := getRec(ctx, tx, cid)
			if err != nil {
				if errors.Is(err, taskgate.ErrTaskNotFound) {
					continue // 子记录没了,没法处理,跳过
				}
				return err
			}
			...
			case taskgate.ChildWake:
				...
				if _, err := tx.ExecContext(ctx,
					`UPDATE tasks SET status = 'pending', pending_parents = ? WHERE id = ?`,
			...
			case taskgate.ChildCancel:
				if c.task.Status == taskgate.StatusRunning {
					...
					if _, err := tx.ExecContext(ctx,
						`UPDATE tasks SET cancel_requested = 1 WHERE id = ?`, cid); err != nil {
						return err
					}
					continue
				}
			...
			default: // ChildNone:计数减了但没减到 0,或子已在终态
				if _, err := tx.ExecContext(ctx,
					`UPDATE tasks SET pending_parents = ? WHERE id = ?`, newPending, cid); err != nil {
					return err
				}
			}
		}
...
func childrenOf(ctx context.Context, tx *sql.Tx, parentID string) ([]string, error) {
	rows, err := tx.QueryContext(ctx,
		`SELECT child_id FROM task_deps WHERE parent_id = ? ORDER BY child_id`, parentID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()

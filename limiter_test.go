...
func TestLimiterBurst(t *testing.T) {
	lim := newLimiter(1, 1, 5) // 每秒才补 1 个,突发额度 5
...
func TestLimiterDefaultBurst(t *testing.T) {
	if got := newLimiter(1, 10, 0).tb.Burst(); got != 10 {
		t.Fatalf("RPS=10 缺省 Burst 应为 10,得到 %d", got)
	}
	if got := newLimiter(1, 0.5, 0).tb.Burst(); got != 1 {
		t.Fatalf("RPS=0.5 缺省 Burst 应为 1,得到 %d", got)
	}
	if got := newLimiter(1, 10, 3).tb.Burst(); got != 3 {

...
已批准的服务器型数据库驱动(均为纯 Go、零 cgo,SQL 后端专用):`github.com/jackc/pgx/v5`(以 `pgx/v5/stdlib` 注册 database/sql 驱动)、`github.com/go-sql-driver/mysql`。新增其他数据库驱动仍需先修宪批准。
...
5. **门控例外(仅限没有纯 Go 内存替身的后端)**:MySQL/PG 这类服务器型后端没有 miniredis 那样能进 CI 的纯 Go 替身,契约测试允许用环境变量门控(如 `TASKGATE_PG_DSN`/`TASKGATE_MYSQL_DSN`),缺失即 skip,不算红;CI 用 service container(postgres、mysql)注入 DSN 后必跑。**代价必须写明并接受**:这是本项目首个"本地零覆盖"的后端——redis 好歹有 miniredis 兜底,而 PG/MySQL 本地 `go test -race` 全绿**不代表**跑过这两个后端,它们的回归唯一防线是 CI。这条例外只对"确实没有离线替身"的后端开口,memory/sqlite/redis 仍须离线全绿,不得借此放松。
...
| 模块路径 | github.com/AmbroseX/taskgate(占位,可改) | 库,非服务 |
| 文件后端 | modernc.org/sqlite | 纯 Go 免 cgo,WAL 模式 |
| Redis 后端 | github.com/redis/go-redis/v9 + go-redis/redis_rate | M2 引入 |
| PG 后端 | github.com/jackc/pgx/v5(stdlib 模式) | 纯 Go 免 cgo,spec 004 引入 |
| MySQL 后端 | github.com/go-sql-driver/mysql | 纯 Go 免 cgo,spec 004 引入 |
...
- v1.2.0 (2026-07-16): MINOR——为 MySQL/PG 后端适配(spec 004)开两处口子:①第 I 条依赖纪律批准两个纯 Go 零 cgo 驱动(pgx/v5 stdlib 模式、go-sql-driver/mysql),技术栈表同步登记;②第 V 条新增第 5 点"门控例外",允许无纯 Go 内存替身的后端(MySQL/PG)用环境变量门控 + CI service container 跑契约,本地缺环境 skip 不算红,并明写"首个本地零覆盖后端、回归唯一防线是 CI"的代价。其余原则原样适用。依据 docs/plans/2026-07-15-MySQL-PG后端适配方案.md 第 6、7 节。

...
- **五种后端一套契约**:`memorybroker`(内存,零依赖)、`sqlitebroker`(单文件落盘,纯 Go 免 cgo)、`redisbroker`(多进程共享,Lua 原子流转)、`pgbroker`(PostgreSQL)、`mysqlbroker`(MySQL)——后两个是服务器型数据库后端,多进程共享,认领互斥靠 `FOR UPDATE SKIP LOCKED`(要求 MySQL 8.0+ / PostgreSQL 9.5+);同一套行为契约(`brokertest` 18 条)五后端验收。
...
五种后端实现同一套 Broker 合同,由同一套 `brokertest`(18 条)验收,换后端只改构造一行、业务代码零改动。按部署形态挑一个:
...
重跑会伤到两类东西,**防护手段完全不同,而且第二类 taskgate 给不了保证**:

| | 本地副作用(扣额度、落库、发通知) | 外部网关调用(调 LLM/OCR) |
|---|---|---|
| 能不能防住 | **能**,用 `Task.ID` 做幂等键 | **取决于网关**,taskgate 无能为力 |
| 手段 | 数据库唯一约束 / 幂等表 / 事务性 outbox | 网关支持 idempotency key 就传 `Task.ID`;不支持只能接受重复调用 |
...
- **网关支持幂等键**(OpenAI 的 `Idempotency-Key`、多数支付类网关都有):把 `t.ID` 传过去,网关自己去重,重跑不产生第二次计费。**这是唯一能真正消除重复调用的办法。**
- **网关不支持**:接受重复调用的成本,或在业务协议层做补偿(对账、按 `t.ID` 追溯去重)。taskgate 帮不上忙——"外部调用已成功、但本地还没落账"的不确定窗口,任何任务队列都消不掉。

> **`WithID` 幂等的是"提交",不是"执行"。** 同 ID 重复 `Submit` 会返回 `ErrTaskExists`,保证任务只入队一次;但入队之后,这一个任务的 handler 仍然可能因为崩溃回收而跑多次。这是两件事,别混。
...
已经有一套 PostgreSQL 或 MySQL,不想再引一个 Redis 时用它:多个 worker 进程接同一个库抢同一批任务,认领互斥靠 `FOR UPDATE SKIP LOCKED`(要求 MySQL 8.0+ / PostgreSQL 9.5+),同样过 18 条契约。和 redis 一样,换后端只改构造函数一行,业务代码零改动:
...
想在真库上再跑一遍 18 条契约(可选;Redis 验证 Lua 脚本兼容性,PG/MySQL 本地无库时自动 skip):

...
- [x] brokertest 17 条在 miniredis 全绿;真 Redis 档(env 门控)全绿
- [x] 单机后端(memory/sqlite)全部 M1 测试零回归
- [x] L3 集成测试 redis 档全绿
- [x] 双进程 1000 任务恰好各执行一次;kill -9 回收重跑;跨进程流水线唤醒;跨进程 Cancel 心跳周期内生效;断连恢复不丢任务
- [x] 分布式限流:双 Gate {Workers:2} 全局并发 ≤2;{RPS:10} 1 秒全局 10±2;崩溃槽 ≤2×LeaseTTL 回收
- [x] 覆盖率:redisbroker ≥80%,核心包维持 ≥85%
- [x] 基准基线数字已回写 docs/plans/2026-07-14-测试方案.md 第 6 节
- [x] `go test ./... -race -count=1` 连跑 3 遍无偶发;gofmt/vet 干净
